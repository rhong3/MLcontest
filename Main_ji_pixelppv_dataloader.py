...
def metric(y_pred, target):
    ...
    #print('predicted ratio:', predicted/len(target_vec))
    return ppv
    ...
    for epoch in range(ep):
        ...
        for itr in range(batches_per_epoch):
            ...
            for iit in range(6):
                ...
                # print(label_ratio)
                if label_ratio < 1:
                    add_weight = (trlaa[0,0,:,:] / 255 + 1/(1/label_ratio - 1))*100
                    loss_fn = torch.nn.BCEWithLogitsLoss(weight=torch.from_numpy(add_weight).type(torch.FloatTensor))
                elif label_ratio > 1:
                    add_weight = (trlaa[0,0,:,:] / 255 + 1/(label_ratio - 1))*100
                    loss_fn = torch.nn.BCEWithLogitsLoss(weight=torch.from_numpy(add_weight).type(torch.FloatTensor))
                elif label_ratio == 1:
        ...
        for itr in range(rows_val):
            ...
            for iit in range(1):
                ...
                # print(label_ratio)
                if label_ratio < 1:
                    add_weight = (valaa[0,0,:,:] / 255 + 1 / (1 / label_ratio - 1)) * 100
                    loss_fn = torch.nn.BCEWithLogitsLoss(weight=torch.from_numpy(add_weight).type(torch.FloatTensor))
                elif label_ratio > 1:
                    add_weight = (valaa[0,0,:,:] / 255 + 1 / (label_ratio - 1)) * 100
                    loss_fn = torch.nn.BCEWithLogitsLoss(weight=torch.from_numpy(add_weight).type(torch.FloatTensor))
                elif label_ratio == 1:
                    loss_fn = torch.nn.BCEWithLogitsLoss()
                # print(add_weight.max(), add_weight.min())
...
def test(tesample, model, group):
    ...
    for itr in range(len(tesample['ID'])):
        ...
        pred_mask = model(xt).cpu()
        # pred_mask = pred_mask(pred_mask > 0.5).type(torch.FloatTensor)
        pred_np = pred_mask.round().data.numpy().astype(np.uint8)
        pred_np = back_scale(pred_np, tedim)
        imsave('../' + output + '/' + group + '/' + teid + '_pred.png', pred_np)

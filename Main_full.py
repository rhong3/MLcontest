    ...
    for epoch in range(ep):
        ...
        for itr in range(batches_per_epoch):
            ...
            for iit in range(6):
                ...
                tr_metric = dice_loss(F.sigmoid(pred_mask), y)
                tr_metric_list.append(tr_metric.cpu().data.numpy()[0])
                # loss += tr_metric
        ...
        for itr in range(vasample.shape[0]):
            ...
            for iit in range(1):
                ...
                va_metric = dice_loss(F.sigmoid(pred_maskv), yv)
                va_metric_list.append(va_metric.cpu().data.numpy()[0])
                # vloss += va_metric
        ...
        print('Epoch {:>3} |lr {:>1.5f} | Loss {:>1.5f} | VLoss {:>1.5f} | Train Score {:>1.5f} | Val Score {:>1.5f} '.format(epoch + 1, lr, lossa, vlossa, 1-tr_score, 1-va_score))
        opt.step()
        opt.zero_grad()
        ...
        if (epoch+1) % 10 == 0:
            ...
            torch.save(checkpoint, '../outputs/unet-{}'.format(epoch+1))
        if epoch > 10:
...
def test(tesample, model, group):
    ...
    for itr in range(tesample.shape[0]):
        ...
        # pred_mask = (pred_mask > 0.25).type(torch.FloatTensor)
        pred_np = pred_mask.data.cpu().numpy()
        pred_np = back_scale(pred_np, tedim)
        imsave('../outputs/'+group+'/'+teid[0]+'_pred.png', pred_np)
...
trsample = pd.read_csv('../inputs/stage_1_train/samples.csv', header = 0, usecols=['Image', 'Label'])
vasample = pd.read_csv('../inputs/stage_1_test/vsamples.csv', header = 0, usecols=['Image', 'Label'])
# trsample = trsample.loc[:2,:]
# vasample = vasample.loc[:2,:]
# teasample = pd.read_csv('../inputs/stage_1_test/samples.csv', header = 0, usecols=['Image', 'ID', 'Width', 'Height'])
tebsample = pd.read_csv('../inputs/stage_2_test/samples.csv', header = 0, usecols=['Image', 'ID', 'Width', 'Height'])
model = train(1, trsample, vasample, 200, 0.01)
# teasub = test(teasample, model, 'stage_1_test')
# teasub.to_csv('../outputs/stage_1_test_sub.csv', index=False)
tebsub = test(tebsample, model, 'stage_2_test')
tebsub.to_csv('../outputs/stage_2_test_sub.csv', index=False)
